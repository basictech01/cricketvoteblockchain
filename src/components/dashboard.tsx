...
async function handleClaimAllRewards(matchId: string, predictions: Prediction[]) {
  try {
    ...
    const totalClaimed = matchPredictions.reduce(
    ...
    const tx = await contract.claimTokens(matchId, totalClaimed, proof);
    await tx.wait();
                                      ...
                                      <CardFooter className="bg-muted/20 p-3 flex justify-between">
                                        ...
                                        {claimableTokens > 0 && (
                                          <Button
                                            ...
                                            disabled={group.predictions.some(
                                              (p) => p.isClaimLoading
                                            )}
                  ...
                  {getClaimableTokens(selectedMatchDetails.predictions) > 0 && (
                    <div className="mt-6 p-4 bg-primary/5 rounded-lg border border-primary/20 flex flex-col sm:flex-row items-center justify-between gap-4">
                      ...
                      <Button
                        ...
                        disabled={selectedMatchDetails.predictions.some(
                          (p) => p.isClaimLoading
                        )}
