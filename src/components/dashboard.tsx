...
export default function DashBoard() {
  ...
  // Clear claim success message after 3 seconds
  useEffect(() => {
    if (claimSuccess) {
  ...
  const handleClaimAllRewards = async (matchId: string) => {
    try {
      // Find all unclaimed winning predictions for this match
      ...
      // Here you would call your API to claim all rewards for this match
      // For example:
      // await fetch('/api/rewards/claim-all', {
      //   method: 'POST',
      ...
      // Update all predictions for this match to show they've been claimed
      setPredictions((prev) =>
        prev.map((p) =>
      ...
      // Set claim success message
      setClaimSuccess(`Successfully claimed ${totalClaimed} CPT tokens!`)
  ...
  const handleClaimReward = async (predictionId: string) => {
    try {
      ...
      setPredictions((prev) => prev.map((p) => (p.id === predictionId ? { ...p, isClaimLoading: true } : p)))

      // Here you would call your API to claim the reward
      // For example:
      // await fetch('/api/rewards/claim', {
      //   method: 'POST',
      //   headers: { 'Content-Type': 'application/json' },
      //   body: JSON.stringify({ predictionId, address })
      // });

      // For now, we'll simulate a successful claim
      await new Promise((resolve) => setTimeout(resolve, 1500))

      // Find the prediction to get its reward amount
      const prediction = predictions.find((p) => p.id === predictionId)
      const rewardAmount = prediction?.reward || 0

      // Update the prediction to show it's been claimed
      setPredictions((prev) =>
        prev.map((p) => (p.id === predictionId ? { ...p, claimed: true, isClaimLoading: false } : p)),
      )

      // Set claim success message
      setClaimSuccess(`Successfully claimed ${rewardAmount} CPT tokens!`)

      toast.success(`Successfully claimed ${rewardAmount} CPT tokens!`)
    } catch (error) {
      console.error("Error claiming reward:", error)
      toast.error("Failed to claim reward. Please try again.")

      // Reset loading state
      setPredictions((prev) => prev.map((p) => (p.id === predictionId ? { ...p, isClaimLoading: false } : p)))
    }
  }
                        ...
                        <AnimatePresence>
                          {claimSuccess && (
                            <motion.div
                              ...
                              animate={{ opacity: 1, y: 0 }}
                              exit={{ opacity: 0, y: -20 }}
                              className="bg-green-100 dark:bg-green-900/30 border-b border-green-200 dark:border-green-800 p-3 flex items-center justify-center gap-2 text-green-800 dark:text-green-300"
                            >
                                    ...
                                    <CardFooter className="bg-muted/20 p-3 flex justify-between">
                                      ...
                                      {claimableTokens > 0 && (
                                        <Button
                                          ...
                                          onClick={() => handleClaimAllRewards(matchId)}
                ...
                <>
                  ...
                  {getClaimableTokens(selectedMatchDetails.predictions) > 0 && (
                    <div className="mt-6 p-4 bg-primary/5 rounded-lg border border-primary/20 flex flex-col sm:flex-row items-center justify-between gap-4">
                      ...
                      <Button
                        onClick={() => handleClaimAllRewards(selectedMatchDetails.matchId)}
